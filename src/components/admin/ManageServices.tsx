  ...
  return (
    <div>
              ...
              <div className="flex items-center justify-end space-x-2">
                ...
                <label className="flex items-center cursor-pointer ml-2" onClick={e => e.stopPropagation()}>
                  <input
                    ...
                    onChange={() => handleToggleShowOnHome(service._id, !!service.showOnHome)}
                    className="sr-only"
                  />
                  <div className={`w-10 h-5 flex items-center rounded-full p-1 duration-300 ease-in-out ${service.showOnHome ? 'bg-green-500' : 'bg-red-500'}`}>
                    <div className={`bg-white w-4 h-4 rounded-full shadow-md transform duration-300 ease-in-out ${service.showOnHome ? 'translate-x-5' : ''}`}></div>
                  </div>
                  <span className={`ml-2 text-xs font-semibold ${service.showOnHome ? 'text-green-400' : 'text-red-400'}`}>{service.showOnHome ? "Visible" : "Hidden"}</span>
                </label>
              </div>
            </div>
          ))}
      ...
      <ConfirmationModal
        ...
        onCancel={() => { setShowDeleteModal(false); setServiceToDelete(null); }}
        confirmText="Delete"
        cancelText="Cancel"
      />
    </div>
