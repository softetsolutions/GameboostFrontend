...
const TrendingServices = () => {
    ...
    const handleCardClick = async (product: Product, title: string) => {
        ...
        try {
            ...
            navigate(`/product?productId=${product._id}&serviceId=${product.service}`, {
    ...
    return (
        <div className="flex flex-col gap-20">
            {sections.map((section) => {
                const products = section.products.map((p) => ({ ...p, service: section._id }));
                const currentStart = startIdx[section._id] || 0;
                const maxIdx = Math.max(0, products.length - CARDS_PER_VIEW);

                return (
                    <div key={section._id} className="mb-12 relative max-w-5xl mx-auto w-full">
                        ...
                        >
                            <div
                                ...
                                style={{
                                    transform: `translateX(-${(currentStart / CARDS_PER_VIEW) * 100}%)`,
